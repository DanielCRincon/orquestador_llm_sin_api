# Orquestador de revisión ABAP multiagente

CLI local enfocado en código ABAP. Ejecuta Codex CLI y Claude Code CLI en paralelo, anonimiza y mezcla sus propuestas antes de enviarlas al juez, y produce un reporte final estructurado. Si el juez detecta una respuesta vacía o un desacuerdo importante, los agentes reciben su evaluación y realizan una ronda adicional de revisión antes de la decisión final. No usa OpenAI API, Anthropic API ni API keys: los CLIs usan sus sesiones autenticadas localmente.
...
El proyecto usa TypeScript 5.9.3 y `@types/node` 22.20.1.
...
También se puede omitir `--file` o repetirlo para varios archivos. Todo informe se escribe en `reports/`; `--out` define únicamente el nombre del archivo:
...
- `CODEX_COMMAND` y `CLAUDE_COMMAND`: ejecutable o ruta local del CLI. En Windows, Codex busca primero una ruta indicada por `CODEX_COMMAND`, después `codex` en el `PATH` y por último la instalación de la extensión de VS Code; Claude usa `claude.exe`. Por seguridad no se aceptan comillas, saltos de línea ni metacaracteres de shell.
- `JUDGE_PROVIDER`: `codex` o `claude`; por defecto `codex`.
...
El juez recibe las respuestas como `PROPUESTA A` y `PROPUESTA B`, en orden aleatorio, sin referencia a Codex ni Claude. Esto reduce sesgo de proveedor. El juez reparte el presupuesto del prompt entre contexto (25 %) y ambas propuestas, por lo que ninguna propuesta desaparece por un truncamiento global. Las salidas que excedan el límite se marcan como truncadas.
...
El proyecto usa resolución de módulos `Node16`, compatible con Node.js 18+ y TypeScript 5.9.3; no requiere silenciar advertencias de deprecación.

El informe indicado con `--out` se guarda bajo `reports/` y contiene respuesta final, consenso, desacuerdos relevantes, rationale, riesgos restantes y pruebas sugeridas. Las respuestas completas de cada agente, las revisiones y los metadatos se guardan en `runs/` para depuración. Cada archivo recibe un UUID para evitar colisiones durante la ejecución paralela. Si un agente devuelve texto vacío, se reintenta una sola vez; si vuelve a ocurrir, la ejecución falla con un diagnóstico.
...
Los directorios `abap/`, `reports/`, `runs/`, `dist/` y `node_modules/`, además de `.env`, están ignorados por Git. No incluyas secretos en problemas, archivos de entrada, nombres de salida ni commits. La licencia del proyecto es [MIT](LICENSE).
